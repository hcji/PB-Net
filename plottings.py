...
def report_scores(test_data, preds):
  dists1 = eval_dists(test_data, preds)
  y_trues = []
  y_preds = []
  for i, sample in enumerate(test_data):
    output = preds[i]
...
# 
#   trainer.load('./model-seq-augmented.pth')
#   pred_shifted2 = trainer.predictv2(test_data_shifted, 32)
#   scores2[i] = report_scores(test_data_shifted, pred_shifted2)
# 
...
# 
#   trainer_ref.load('./model-ref-augmented.pth')
#   pred_shifted_ref2 = trainer_ref.predictv2((test_data_shifted, ref), 32)
#   scores_ref2[i] = report_scores(test_data_shifted, pred_shifted_ref2)
# 
#   with open('./utils/rt_shift_trend.pkl', 'wb') as f:
#     pickle.dump([scores_bl, scores, scores2, scores_ref, scores_ref2], f)

with open('./utils/rt_shift_trend.pkl', 'rb') as f:
  scores_bl, scores, scores2, scores_ref, scores_ref2 = pickle.load(f)

x = np.arange(-10, 11)
y_bl = np.array([scores_bl[i][0] for i in x])
y_seq = np.array([scores[i][0] for i in x])
y_seq_aug = np.array([scores2[i][0] for i in x])
y_ref = np.array([scores_ref[i][0] for i in x])
y_ref_aug = np.array([scores_ref2[i][0] for i in x])

plt.clf()
plt.plot(x*0.6, y_bl, 'o-', markersize=5, color=color_bl, label='baseline')
plt.plot(x*0.6, y_seq, 'o-', markersize=5, color=color_seq, label='Seq PB-Net')
plt.plot(x*0.6, y_seq_aug, 'v:', markersize=5, color=color_seq, label='Seq PB-Net(augmented)')
plt.plot(x*0.6, y_ref, 'o-', markersize=5, color=color_ref, label='Ref PB-Net')
plt.plot(x*0.6, y_ref_aug, 'v:', markersize=5, color=color_ref, label='Ref PB-Net(augmented)')
...
y_seq = np.array([scores[i][2] for i in x])
y_seq_aug = np.array([scores2[i][2] for i in x])
y_ref = np.array([scores_ref[i][2] for i in x])
y_ref_aug = np.array([scores_ref2[i][2] for i in x])
